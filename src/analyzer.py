...
class AnalysisResult(BaseModel):
    ...
    reason: str = Field(description="判断理由（連鎖分析はA→B→Cの形式）", max_length=400)
...
- 川崎重工業 (7012)
- 東京応化工業 (4186) ← 半導体材料メーカー（フォトレジスト世界トップ）
- 三菱重工業 (7011)
...
【★最重要★ サプライチェーン連想分析】
ニュースが直接 保有株に言及していなくても、サプライチェーンの上流/下流の動きから
保有株への影響を「連想」して分析してください。特に以下の連鎖を常に意識すること:

＜半導体サプライチェーン連鎖マップ＞
SOX指数は東京応化(4186)の「先行指標」。SOX構成企業が半導体を大量生産 → 材料のフォトレジストが消費される。

【ポジティブ連鎖（BUY/STRONG_BUY方向）】
  ① NVIDIA好決算/GPU需要増 → AIチップ生産増 → 最先端3nm/5nmプロセス稼働率UP
    → EUV露光回数増 → フォトレジスト(東京応化の主力製品)バカ売れ → 東京応化(4186)にSTRONG_BUY

  ② TSMC月次売上増/設備投資増/新工場建設(熊本JASM等) → 製造キャパ拡大
    → フォトレジスト消費量増 → 東京応化(4186)にBUY（TSMCは東京応化の最大顧客級）

  ③ Intel復権/新工場稼働/AMD好決算 → PC・サーバー半導体増産
    → フォトレジスト消費量増 → 東京応化(4186)にBUY

  ④ ASML受注増/EUV装置出荷増 → 先端プロセス拡大 → EUV用レジスト需要増
    → 東京応化(4186)にBUY（東京応化はEUV用フォトレジスト世界トップシェア）

  ⑤ SOX指数上昇 → 半導体セクター全体が強気 → 日本の半導体関連株も連動上昇
    → 東京応化(4186)にBUY

  ⑥ 対中半導体規制強化 → TSMC/Samsung先端工場に生産シフト → 先端品需要増
    → 東京応化(4186)にBUY（ただし中国向け売上減リスクも注記）

  ⑦ 生成AIブーム加速(OpenAI新モデル/ChatGPT等) → GPU需要爆発 → ①に連鎖

【ネガティブ連鎖（SELL方向）】
  ⑧ SOX指数急落 / NVIDIA業績悪化 → 半導体セクター全体にネガティブ
    → 東京応化(4186)にもマイナス → SELL検討

  ⑨ iPhone出荷減/スマホ・PC需要減退 → 半導体メーカーが減産・稼働率低下
    → フォトレジスト消費量減 → 東京応化(4186)にマイナス → SELL検討

  ⑩ メモリ在庫調整(Micron/SK hynix等) → メモリ工場稼働率低下
    → 東京応化(4186)の出荷減 → SELL検討（これを「連れ安」と言う）

＜防衛サプライチェーン連鎖マップ＞
  台湾有事リスク上昇 / 中国軍事行動 → 防衛費増額期待
    → 三菱重工業(7011), 川崎重工業(7012) にプラス

  NATO / 欧州防衛費増 → グローバル防衛産業活況
    → 三菱重工業(7011), 川崎重工業(7012) にプラス

このように、ニュースが「NVIDIA」「Intel」「TSMC」「AMD」「SOX指数」「GPU」等に言及している場合、
必ず東京応化工業(4186)への波及効果を分析し、tickerに含めること。
reason にも「NVIDIA好決算→EUV需要増→東京応化にプラス」のような連鎖を明記すること。
...

{
  ...
  "reason": "このニュースでどの株が上がる/下がるか。連鎖があれば「A→B→C→保有株にプラス/マイナス」の形式で（日本語）",
...
- ★ 連想分析の場合もticker欄には波及先の保有株を含めること
...
- "様子見" : 方向感なし、焦る必要なし
保有株に間接的に影響する連想ニュースの場合も、holding_action を記入すること。
保有株に全く関係ないニュースの場合は空文字""でOK
